...
#     input_base_fname = fname[:2] + '0' + fname[2:].replace('.trans', '.wav')
#     input_fname = os.path.join('..', 'raw_data', 'swb_sph', input_base_fname)
...
def get_one_audio(idx, row):
    ...
    # 1. Correctly constructs the full 7-character ID (sw02005)
    conv_id = fname[:2] + '0' + fname[2:].replace('.trans', '') 
    
    # 2. CONSTRUCTS THE CORRECT INPUT FILENAME (sw02005A.wav or sw02005B.wav)
    input_base_fname = f"{conv_id}{speaker}.wav"
    input_fname = os.path.join('..', 'raw_data', 'swb_sph', input_base_fname)
    ...
    # 3. Uses simple 'sox trim' command on the already single-channel file
    # Add the '-r 8000' option before the output file name to ensure the output is always 8kHz
    cmd = 'sox ' + input_fname + ' ' + output_fname + ' rate 8000 trim ' + start + ' ' + dur
    os.system(cmd)
...
def get_audio():
    ...
    Parallel(n_jobs=-1)(delayed(get_one_audio)(a, b) for a, b in params)
...
def add_silences():
    ...
    Parallel(n_jobs=-1)(delayed(add_one_silence)(a) for a in params)
