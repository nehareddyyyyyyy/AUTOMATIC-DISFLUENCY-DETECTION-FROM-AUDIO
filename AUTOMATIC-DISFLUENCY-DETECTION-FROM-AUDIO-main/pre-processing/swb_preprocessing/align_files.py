...
def scrub_word(word):
    ...
    if re.match(r"(\[\w+-)?([0-9a-zA-Z-']+)(\[[0-9a-zA-Z-']+\])(-?)(\])", word) is not None:
        word = re.sub(r"(\[\w+-)?([0-9a-zA-Z-']+)(\[[0-9a-zA-Z-']+\])(-?)(\])", "\\2" + "\\4", word)
    elif re.match(r"[\[]\w+-[0-9a-zA-Z-']+[\]]", word) is not None:
        word = re.sub(r"[\[]\w+-([0-9a-zA-Z-']+)[\]]", "\\1", word)
    else:
        word = re.sub(r"[\[].*?[\]]", "", word)

    word = re.sub(r"(\w+)(_.*)", "\\1", word)  # _1 -->

    word = re.sub(r"\{([0-9a-zA-Z-']+)\}", "\\1", word)  # {federale} --> federale

    word = re.sub(r"-\/([0-9a-zA-Z-']+)\]", "\\1", word)  # [csh[ape]-/shape] --> shape
    word = re.sub(r"(\w+)/(\w+)\]", "\\2", word)  # [-[Can]sego/Canseco] --> Canseco
...
def align_files():
    ...
    with open('/home/saikrishnanair/PS 3-1/disfluency_detection_from_audio/raw_data/swb_swapped_speak.txt', 'r') as fp:
    ...
    for audio_fname in fnames:
        ...
        for speaker in ['A', 'B']:
            ...
            trans = [re.sub(r'\s+', ' ', trans_line.strip('\n')) for trans_line in trans]
